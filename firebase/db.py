...
def _safe_dt(val):
    try:
        ...
        if hasattr(val, 'timestamp'):
            return datetime.fromtimestamp(float(val.timestamp()), tz=UTC)
    except Exception:
        pass
    return datetime.min.replace(tzinfo=UTC)
...
def create_user(tg_id: int, name: str, username: str = None, role: str = "user") -> dict:
    data = {
        ...
        "created_at": datetime.now(UTC), "last_active": datetime.now(UTC),
    }
...
def block_user(tg_id: int, blocked: bool = True):
    get_db().collection("users").document(str(tg_id)).update({
        "is_blocked": blocked, "last_active": datetime.now(UTC),
    })
...
def get_test(tid: str):
    ...
    if not doc.exists:
        return None
    d = doc.to_dict()
    return None if not d.get("is_active", True) else d
...
def get_public_tests(limit: int = 100) -> list:
    docs = [d.to_dict() for d in
            ...
            .where("visibility", "==", "public").limit(150).stream()]
    docs = [d for d in docs if d.get("is_active", True)]
    docs.sort(key=lambda x: _safe_dt(x.get("created_at")), reverse=True)
    return docs[:limit]
...
def get_my_tests(creator_id: int) -> list:
    docs = [d.to_dict() for d in
            get_db().collection("tests")
            .where("creator_id", "==", creator_id).stream()]
    docs = [d for d in docs if d.get("is_active", True)]
    docs.sort(key=lambda x: _safe_dt(x.get("created_at")), reverse=True)
    return docs
...
def _update_user_stats(uid: int, pct: float):
    ...
    ref.update({
        "total_tests": total, "total_score": score,
        "avg_score": round(score / total, 1), "last_active": datetime.now(UTC),
    })
    ...
    if doc.exists:
        if pct > doc.to_dict().get("best_percentage", 0):
            db.collection("leaderboard").document(lid).update({
                "best_percentage": pct, "updated_at": datetime.now(UTC),
            })
    else:
        db.collection("leaderboard").document(lid).set({
            "user_id": uid, "user_name": name, "test_id": tid,
            "best_percentage": pct,
            "created_at": datetime.now(UTC), "updated_at": datetime.now(UTC),
        })
