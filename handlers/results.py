...
async def profile_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    role_text = {"admin": "👑 Admin", "teacher": "👨‍🏫 O'qituvchi", "user": "👤 Foydalanuvchi"}.get(role, "👤")
    ...
    if recent_results:
        ...
        for r in recent_results:
            ...
            recent_text += f"  {status} {title} — {pct:.0f}%\n"
    
    text = f"""
👤 <b>MENING PROFILIM</b>

{role_text}
📛 Ism: <b>{user.get('name', 'Noma\'lum')}</b>
🆔 ID: <code>{user_id}</code>

━━━━━━━━━━━━━━━
📊 Statistika:
📝 Ishlagan testlar: <b>{total_tests}</b>
📈 O'rtacha natija: <b>{avg_score:.1f}%</b>
🏅 Badgelar: <b>{len(badges)}</b>
━━━━━━━━━━━━━━━
{recent_text}
"""
...
async def my_results_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard_rows.append([InlineKeyboardButton("🏠 Bosh sahifa", callback_data="main_menu")])
    
    if query:
        await query.message.edit_text(text, reply_markup=InlineKeyboardMarkup(keyboard_rows), parse_mode="HTML")
    else:
